...
class ListHeadlinesFragment : Fragment() {
    ...
    private fun observeUiEvent() {
        viewModel.getEvent().observe(viewLifecycleOwner) { event ->
            when (event) {
                is ListHeadlinesEvent.GetListNews -> setListNews(event.articles)
                is ListHeadlinesEvent.ArticleClicked -> openFragmentNews(event.bundle)
                is ListHeadlinesEvent.GetAllArticles -> setListNews(event.articles)
                else -> {}
            }
        }
    }
    ...
    private fun setListNews(articles: List<Article>) {
        adapter.setListNews(articles)
    }
